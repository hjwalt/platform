...
func NewExploder[IV any, OV any, ERR any](
	name string,
	handler Explode[IV, OV, ERR],
	...
	if result.IsPresent() {
		for _, entry := range result.Get() {
			outputMessage := flow.Message[OV]{
				Metadata:  r.OutputMetadata(ctx, msg.Metadata, entry),
				Value:     entry,
				Timestamp: time.Now(),
			}
			if err := r.OutputProducer.ProduceMessage(ctx, []flow.Message[OV]{outputMessage}); err != nil {
				return err
			}
		}
	}
